...
logger.pop "A111 WebUIランチャーを読み込み中"
...
if (Test-Path $configFile) {
    logger.info "カスタムWebUIコンフィグが見つかりました"
    $webUIConfig = Get-Content -Path $configFile | ConvertFrom-Json
}

if ($args.Length -gt 0) {
    logger.info "ランチャーの起動オプション : $args"
}
else {
    logger.info "ランチャーの起動オプションがありません"
}

$GPUInfo = Get-GPUInfo
$GPUText = "互換性のあるGPUがありません"
if ($GPUInfo) {
    $GPUText = "$($GPUInfo.Model) $($GPUInfo.VRAM) GB"
}
logger.space "White"
logger.action "要求事項の確認:"
Install-py
Install-git
Install-WebUI
Import-BaseModel
...
function Makeform {
    ...
    $versionLabel.Text = "ランチャー V$($Version.Long)"
    ...
    $HashLabel.Text = "WebUIのハッシュ(Click to copy) : $HashText"
